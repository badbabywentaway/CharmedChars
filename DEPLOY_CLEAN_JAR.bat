@echo off
echo ===================================================
echo    CharmedChars v1.3.2 - Clean Deployment Script
echo ===================================================
...
echo Step 6A: Copying new JAR to ItemsAdder server (v1.3.2)...
copy "build\libs\CharmedChars-1.3.2.jar" "c:\Users\steve\Documents\Papermc\plugins\" /Y
if %ERRORLEVEL% EQU 0 (
...
echo Step 6B: Copying new JAR to Oraxen server (v1.3.2)...
copy "build\libs\CharmedChars-1.3.2.jar" "c:\Users\steve\Documents\OraxenPapermc\plugins\" /Y
if %ERRORLEVEL% EQU 0 (
...
echo Step 6C: Copying new JAR to Nexo server (v1.3.2)...
copy "build\libs\CharmedChars-1.3.2.jar" "c:\Users\steve\Documents\NexoPapermc\plugins\" /Y
if %ERRORLEVEL% EQU 0 (
...
echo.
echo ===================================================
echo [SUCCESS] CharmedChars v1.3.2 deployed successfully!
echo ===================================================
...
echo ===================================================
echo NEW in 1.3.2:
echo   - CRITICAL: Number scoring now works on ORAXEN servers!
echo   - Added PlayerInteractEvent support (Oraxen compatibility fix)
echo   - Number blocks now drop as items in Overworld/End
echo   - Consistent block breaking with correct tools across all providers
echo   - Matches letter block behavior exactly (dual event handlers)
echo   - Debug logging for event handler troubleshooting
echo.
echo Previous 1.3.1 features:
echo   - Number scoring happens on HIT (like letters)
echo   - Fixed hardcoded namespace bug in number recognition
echo   - Number blocks work with all providers (ItemsAdder/Oraxen/Nexo)
