...
class OraxenSetup(private val plugin: CharmedChars) {
    ...
    private fun generateItemsConfig(): Boolean {
        try {
            ...
            yaml.appendLine("# Total: 131 items (126 blocks + 5 pyrite items)")
            yaml.appendLine("")
            ...
            for (color in colors) {
                ...
                for (letter in 'a'..'z') {
                    ...
                    yaml.appendLine("          - WOODEN_AXE")
                    yaml.appendLine("          - STONE_PICKAXE")
                    yaml.appendLine("          - STONE_AXE")
                    yaml.appendLine("          - COPPER_PICKAXE")
                    yaml.appendLine("          - COPPER_AXE")
                    yaml.appendLine("          - IRON_PICKAXE")
                    yaml.appendLine("          - IRON_AXE")
                    yaml.appendLine("          - GOLDEN_PICKAXE")
                    yaml.appendLine("          - GOLDEN_AXE")
                    yaml.appendLine("          - DIAMOND_PICKAXE")
                    yaml.appendLine("          - DIAMOND_AXE")
                    yaml.appendLine("          - NETHERITE_PICKAXE")
                    yaml.appendLine("          - NETHERITE_AXE")
            ...
            // Generate logo blocks
            for (color in colors) {
                val displayColor = colorDisplayNames[color]
                yaml.appendLine("# ==================== ${color.uppercase()} LOGO BLOCK ====================")
                yaml.appendLine("${color}_logo:")
                yaml.appendLine("  displayname: \"$displayColor Logo Block\"")
                yaml.appendLine("  material: PAPER")
                yaml.appendLine("  Pack:")
                yaml.appendLine("    generate_model: false")
                yaml.appendLine("    model: item/${color}_logo")
                yaml.appendLine("  Mechanics:")
                yaml.appendLine("    noteblock:")
                yaml.appendLine("      custom_variation: ${customVariation++}")
                yaml.appendLine("      directional: false")
                yaml.appendLine("      hardness: 0.8")
                yaml.appendLine("      block_model: block/${color}_logo")
                yaml.appendLine("      drop:")
                yaml.appendLine("        silktouch: false")
                yaml.appendLine("        minimal_type:")
                yaml.appendLine("          - WOODEN_PICKAXE")
                ...
                yaml.appendLine("          - NETHERITE_AXE")
                yaml.appendLine("        loots:")
                yaml.appendLine("          - { oraxen_item: ${color}_logo, probability: 1.0 }")
                yaml.appendLine("")
            }
