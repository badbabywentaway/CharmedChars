...
powershell -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip = [System.IO.Compression.ZipFile]::OpenRead('CharmedChars-temp.zip'); $entries = $zip.Entries | Where-Object {$_.FullName -like 'pack/assets/minecraft/textures/cyan/*.png'}; Write-Host '=== CYAN TEXTURES IN JAR ==='; Write-Host ''; $uppercase = 0; $lowercase = 0; foreach ($e in $entries) { $name = Split-Path $e.FullName -Leaf; if ($name -cmatch '^[A-Z]\.png$') { Write-Host '[ERROR] UPPERCASE:' $name -ForegroundColor Red; $uppercase++ } elseif ($name -cmatch '^[a-z]\.png$') { Write-Host '[OK] lowercase:' $name -ForegroundColor Green; $lowercase++ } else { Write-Host '[INFO]' $name -ForegroundColor Gray } }; Write-Host ''; Write-Host 'Summary:'; Write-Host '  Uppercase letter files:' $uppercase; Write-Host '  Lowercase letter files:' $lowercase; Write-Host ''; if ($uppercase -gt 0) { Write-Host '[ERROR] JAR contains UPPERCASE files - rebuild needed!' -ForegroundColor Red; exit 1 } else { Write-Host '[OK] No uppercase files found!' -ForegroundColor Green; exit 0 }; $zip.Dispose()"

set JAR_STATUS=%ERRORLEVEL%

echo.
echo Step 3: Checking block models...
powershell -Command "Add-Type -AssemblyName System.IO.Compression.FileSystem; $zip = [System.IO.Compression.ZipFile]::OpenRead('CharmedChars-temp.zip'); $entries = $zip.Entries | Where-Object {$_.FullName -like 'pack/models/block/cyan/*.json'}; Write-Host '=== CYAN BLOCK MODELS IN JAR ==='; Write-Host ''; foreach ($e in $entries) { $name = Split-Path $e.FullName -Leaf; if ($name -cmatch '^[A-Z]\.json$') { Write-Host '[ERROR] UPPERCASE:' $name -ForegroundColor Red } elseif ($name -cmatch '^[a-z]\.json$') { Write-Host '[OK] lowercase:' $name -ForegroundColor Green } }; $zip.Dispose()"
